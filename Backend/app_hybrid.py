...
ROBERTA_WEIGHT = 0.85  # RoBERTa gets 85% weight
ML_WEIGHT = 0.15       # ML models get 15% weight
...
def predict():
    ...
    try:
        ...
        result = {
            ...
            'human_probability': 1.0 - final_ai_prob,
            'label': 'AI' if is_ai else 'Human',
            'model_name': model_name,
            'breakdown': {
                'roberta_prob': roberta_ai_prob,
                'ml_prob': ml_ai_prob if ml_ensemble else None,
                'roberta_weight': ROBERTA_WEIGHT,
                'ml_weight': ML_WEIGHT if ml_ensemble else 0
            }
        }
